...
def get_rank_list(dist_vec, q_id=None, q_cam=None, g_ids=None, g_cams=None,
    ...
    """
    sort_inds = np.argsort(dist_vec) #对一个query与gallery的距离进行排序 argsort()函数是将x中的元素从小到大排列，提取其对应的index(索引号)
    ...
    same_id = []
    i = 0
    for ind in sort_inds:#进入到这里 遍历sort_inds19732个人
        g_id, g_cam = g_ids[ind], g_cams[ind] #获得索引号获取的id和摄像头号
        ...
        rank_list.append(ind) #将索引号添加到rank_list
        i += 1 #10个不同摄像头的相同的一个人
        if i >= rank_list_size:
            break
    return rank_list, same_id #rank_list存储这10个索引号
...
def save_rank_list_to_im(rank_list, q_im_path, g_im_paths, save_path, same_id=None, resize_h_w=(128, 64)):
    ...
    for i, ind in enumerate(rank_list):
        ...
        if same_id is not None:
            ...
            im = add_border(im, 3, color) #正确样本增加绿色边框
        ims.append(im)
    im = make_im_grid(ims, 1, len(rank_list) + 1, 8, 255) #将图片按照一行10列进行排列好显示
