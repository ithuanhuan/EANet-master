...
def to_tensor(in_dict, cfg):
    in_dict['im'] = F.to_tensor(in_dict['im']) #转换为张量类型
    in_dict['im'] = F.normalize(in_dict['im'], cfg.im.mean, cfg.im.std) #通过均值，方差正则化
    if 'pap_mask' in in_dict:
...
def transform(in_dict, cfg):
    for t in cfg.transform_list: #t = 'resize'和 'hflip'
        eval('{}(in_dict, cfg)'.format(t))
    to_tensor(in_dict, cfg) #将图片特征转换为张量
    return in_dict #将in_dict的图片特征转换为正则化的张量
