...
class PSLoss(Loss):
    ...
    def __call__(self, batch, pred, step=0, **kwargs):
        ...
        loss = self.criterion(ps_pred, ps_label) #预测的部位和lable部位比较计算损失
        ...
        if cfg.normalize_size:
            ...
            for i in range(cfg.num_classes): #cfg.num_classes
                loss_i = loss[ps_label == i] # ps_label == i 结果为tensor  loss[ps_label == i] 结果也为tensor
                if loss_i.numel() > 0: #loss_i.numel() = 1375
                    loss_ += loss_i.mean()
        ...
        self.meter_dict[cfg.name].update(loss.item())  #self.meter_dict['cd_psL'] = {RecentAverageMeter} self.meter_dict['cd_psL'].avg = 2.07 self.meter_dict['cd_psL'].val = 2.07
        ...
        loss *= cfg.weight #loss = 2.079 weight =1
