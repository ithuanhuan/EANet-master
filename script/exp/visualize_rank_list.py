...
def main(args):
    ...
    ###################
    q_feat = trainer.infer_im_list(q_im_paths, get_pap_mask=get_pap_mask if args.pap_mask_provided else None) #q_feat存储着16张图片的路径和特征
    g_feat = trainer.infer_im_list(g_im_paths, get_pap_mask=get_pap_mask if args.pap_mask_provided else None) #19732张图片的路径和特征
    if 'visible' in q_feat:
        dist_mat = compute_dist_with_visibility(q_feat['feat'], g_feat['feat'], q_feat['visible'], g_feat['visible'])
    else:#到了这里
        dist_mat = compute_dist(q_feat['feat'], g_feat['feat']) #存储这16张图片和19732张图片之间的距离
    g_ids, g_cams = zip(*[parse_im_path(g_im_path) for g_im_path in g_im_paths]) if args.id_aware else (None, None) #g_ids, g_cams存储着gallery的id和摄像头
    for i, (dist_vec, q_im_path) in enumerate(zip(dist_mat, q_im_paths)):#dist_mat遍历16张图片的路径和对应的16张图片和gallery之间的距离，q_im_paths存储这16张图片特征，dist_vec存储着一个query即q_im_path和19732张图片的距离
        q_id, q_cam = parse_im_path(q_im_path) if args.id_aware else (None, None) #进入到这里 q_ids parse_im_path(q_im_path)返回q_im_path这张图片对应的人id和摄像头
        rank_list, same_id = get_rank_list(dist_vec, q_id, q_cam, g_ids, g_cams, args.rank_list_size, id_aware=args.id_aware)#rank_list存储着10个gallery图片和query相似的
